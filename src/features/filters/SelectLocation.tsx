...
function SelectLocation(): React.JSX.Element {
  const { selectLocationCurVal } = useSelector(getFilters);
  const dispatch = useDispatch();

  const { isLoading, locations, errorLocations } = useLocations();

  function handleChange(location: string): void {
    const locationId = locations?.find((locationObj: Location) => locationObj.location === location)
      ?.id;
  ...
  if (errorLocations && errorLocations instanceof Error) {
    return <span>{errorLocations.message}</span>;
  }

  const locationsEdited = locations?.map((locationObj: Location) => ({
    id: locationObj.id,
  ...
  return (
    <div className={styles.selectContainer}>
      <Select
        ...
        disabled={isLoading}
        options={locationsEdited!}
        value={selectLocationCurVal}
